[**@apihive/core**](../README.md)

***

[@apihive/core](../modules.md) / index

# index

## Classes

- [HTTPRequestFactory](classes/HTTPRequestFactory.md)

## Interfaces

- [Adapter](interfaces/Adapter.md)
- [AdapterEntry](interfaces/AdapterEntry.md)
- [AdapterOptions](interfaces/AdapterOptions.md)
- [AdapterPriority](interfaces/AdapterPriority.md)
- [Feature](interfaces/Feature.md)
- [RequestInterceptorControls](interfaces/RequestInterceptorControls.md)
- [ResponseInterceptorControls](interfaces/ResponseInterceptorControls.md)

## Type Aliases

- [APIConfig](type-aliases/APIConfig.md)
- [BaseAPIInterface](type-aliases/BaseAPIInterface.md)
- [BeforeFetchHook](type-aliases/BeforeFetchHook.md)
- [DefaultAPIConfig](type-aliases/DefaultAPIConfig.md)
- [DynamicHeaderValue](type-aliases/DynamicHeaderValue.md)
- [Endpoint](type-aliases/Endpoint.md)
- [EndpointParams](type-aliases/EndpointParams.md)
- [EndpointParamsTemplate](type-aliases/EndpointParamsTemplate.md)
- [ErrorInterceptor](type-aliases/ErrorInterceptor.md)
- [ExpectedResponseFormat](type-aliases/ExpectedResponseFormat.md)
- [FeatureCommands](type-aliases/FeatureCommands.md)
- [FeatureFactoryDelegates](type-aliases/FeatureFactoryDelegates.md)
- [FeatureName](type-aliases/FeatureName.md)
- [FeatureRequestDelegates](type-aliases/FeatureRequestDelegates.md)
- [FetchLike](type-aliases/FetchLike.md)
- [HeaderValue](type-aliases/HeaderValue.md)
- [HTTPMethod](type-aliases/HTTPMethod.md)
- [LiteralValue](type-aliases/LiteralValue.md)
- [MaybeGetterFunction](type-aliases/MaybeGetterFunction.md)
- [NamedEndpoint](type-aliases/NamedEndpoint.md)
- [ProgressHandler](type-aliases/ProgressHandler.md)
- [ProgressHandlerConfig](type-aliases/ProgressHandlerConfig.md)
- [ProgressInfo](type-aliases/ProgressInfo.md)
- [ProgressPhase](type-aliases/ProgressPhase.md)
- [QueryParameterValue](type-aliases/QueryParameterValue.md)
- [RequestConfig](type-aliases/RequestConfig.md)
- [RequestConfigBuilder](type-aliases/RequestConfigBuilder.md)
- [RequestInterceptor](type-aliases/RequestInterceptor.md)
- [ResponseBodyTransformer](type-aliases/ResponseBodyTransformer.md)
- [ResponseHandler](type-aliases/ResponseHandler.md)
- [ResponseInterceptor](type-aliases/ResponseInterceptor.md)
- [ResponseInterceptorWithOptions](type-aliases/ResponseInterceptorWithOptions.md)
- [URLParams](type-aliases/URLParams.md)
