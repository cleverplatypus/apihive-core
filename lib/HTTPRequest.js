...
export class HTTPRequest {
    ...
    setupQueryParams() {
        if (Object.keys(this.config.queryParams).length) {
            ...
            for (let k of Object.keys(this.config.queryParams)) {
                const value = maybeFunction(this.config.queryParams[k], this);
                params[k] = value;
            }
                ...
                for (const entry of this.config.responseInterceptors) {
                    const { interceptor, skipTransformersOnReturn } = typeof entry === "function"
                        ...
                        : {
                            interceptor: entry
                                .interceptor,
                            skipTransformersOnReturn: (_a = entry
                                .skipTransformersOnReturn) !== null && _a !== void 0 ? _a : false,
                        };
    ...
    getHash() {
        ...
        for (const headerKey of relevantHeaderKeys) {
            const headerValue = this.config.headers[headerKey] ||
