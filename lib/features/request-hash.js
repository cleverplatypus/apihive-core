...
function isBinaryBody(body, contentType, textualMimeTypes) {
    ...
    if (body instanceof FormData)
        return formDataHasBinary(body);
    // Heuristic by content-type when body type isn’t conclusive
    if (contentType) {
        const ct = contentType.toLowerCase();
        const textualCT = textualMimeTypes === null || textualMimeTypes === void 0 ? void 0 : textualMimeTypes.some((mt) => new RegExp(mt).test(ct));
        return !textualCT;
    }
    return false;
}
...
class RequestHashFeature {
    constructor() {
        this.name = "request-hash";
        ...
        return {
            request: {
                getHash: (request) => {
                    ...
                    const isBinary = isBinaryBody(bodyContent, contentType, [
                    ...
                    if (!config.finalURL) {
                        throw new Error("Request URL is not finalised. Hashing is only valid after URL finalisation (happens during execute()).");
                    }
...
//# sourceMappingURL=request-hash.js.map